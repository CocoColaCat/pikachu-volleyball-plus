...
export class PikachuVolleyball {
  ...
  startOfNewGame() {
    if (this.frameCounter === 0) {
      ...
      this.physics.initializeBallsForNewRound(this.isPlayer2Serve);
  ...
  round() {
    ...
    if (
      ...
      const ballTouchingGround =
        this.physics.lastBallTouchingGround || this.physics.ball;
      if (ballTouchingGround.punchEffectX < GROUND_HALF_WIDTH) {
  ...
  /** @return {number} */
  get ballCount() {
    return this.physics.activeBallCount;
  }

  /**
   * @param {number} ballCount number of balls in play
   */
  set ballCount(ballCount) {
    this.physics.setBallCount(ballCount);
    this.physics.initializeBallsForNewRound(this.isPlayer2Serve);
    this.view.game.drawPlayersAndBall(this.physics);
  }
}
