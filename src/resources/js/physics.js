...
export class PikaPhysics {
  ...
  constructor(isPlayer1Computer, isPlayer2Computer) {
    ...
    this.balls = [new Ball(false, 0), new Ball(false, 1)];
    this.ball = this.balls[0];
    this.activeBallCount = 1;
    this.lastBallTouchingGround = null;
  }
  ...
  runEngineForNextFrame(userInputArray) {
    const ballTouchingGround = physicsEngine(
      this.player1,
      this.player2,
      this.getActiveBalls(),
      userInputArray
    );
    this.lastBallTouchingGround = ballTouchingGround;
    return ballTouchingGround !== null;
  }

  /**
   * Initialize all balls for a new round.
   * @param {boolean} isPlayer2Serve Will player 2 serve on this new round?
   */
  initializeBallsForNewRound(isPlayer2Serve) {
  ...
  /**
   * Set how many balls are active in the game.
   * @param {number} ballCount
   */
  setBallCount(ballCount) {
    this.activeBallCount = Math.max(1, Math.min(this.balls.length, ballCount));
  }

  /**
   * Get balls currently used by the game.
   * @return {Ball[]}
   */
  getActiveBalls() {
    return this.balls.slice(0, this.activeBallCount);
  }
}
...
class Ball {
   ...
   * @param {boolean} isPlayer2Serve Will player 2 serve on this new round?
   */
  constructor(isPlayer2Serve, ballIndex) {
  ...
  initializeForNewRound(isPlayer2Serve) {
    ...
    if (this.ballIndex === 1) {
      this.x = GROUND_WIDTH - this.x;
    }
    /** @type {number} y coord */
    this.y = this.ballIndex === 1 ? 40 : 0; // 0x34   // initialized to 0
    /** @type {number} x direction velocity */
    this.xVelocity = 0; // 0x38  // initialized to 0
    /** @type {number} y direction velocity */
    this.yVelocity = this.ballIndex === 1 ? 2 : 1; // 0x3C  // initialized to 1
 ...
 * @param {Ball[]} balls balls
 * @param {PikaUserInput[]} userInputArray userInputArray[0]: user input for player 1, userInputArray[1]: user input for player 2
 * @return {Ball|null} Ball touching ground
 */
function physicsEngine(player1, player2, balls, userInputArray) {
  let ballTouchingGround = null;
  for (const ball of balls) {
    const isBallTouchingGround =
      processCollisionBetweenBallAndWorldAndSetBallPosition(ball);
    if (isBallTouchingGround === true && ballTouchingGround === null) {
      ballTouchingGround = ball;
    }
  }
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const targetBall = chooseTargetBallForPlayer(player, balls);
    calculateExpectedLandingPointXFor(targetBall); // calculate expected_X;
  ...
  return ballTouchingGround;
}

/**
 * Choose the ball this player should react to.
 * @param {Player} player
 * @param {Ball[]} balls
 * @return {Ball}
 */
function chooseTargetBallForPlayer(player, balls) {
  if (balls.length === 1) {
    return balls[0];
  }
  ...
  const rightBoundary = leftBoundary + GROUND_HALF_WIDTH;

  for (const ball of balls) {
    calculateExpectedLandingPointXFor(ball);
    const isExpectedOnPlayerSide =
      ball.expectedLandingPointX > leftBoundary &&
      ball.expectedLandingPointX < rightBoundary;
    const isCurrentlyOnPlayerSide =
      ball.x > leftBoundary && ball.x < rightBoundary;
    const sidePriority =
      Number(!isExpectedOnPlayerSide) * 400 +
      Number(!isCurrentlyOnPlayerSide) * 160;
    const fallingPriority = ball.yVelocity > 0 ? 0 : 80;
    const distancePriority = Math.abs(ball.expectedLandingPointX - player.x);
    const heightPriority = Math.max(0, 180 - ball.y);
    const score =
      sidePriority + fallingPriority + distancePriority + heightPriority;
