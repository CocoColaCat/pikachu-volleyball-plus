...
export class GameView {
  ...
  constructor(resources) {
    ...
    this.balls = [
      makeBallAnimatedSprites(textures),
      makeBallAnimatedSprites(textures),
    ];
    this.ballHypers = [
      makeSpriteWithAnchorXY(textures, TEXTURES.BALL_HYPER, 0.5, 0.5),
      makeSpriteWithAnchorXY(textures, TEXTURES.BALL_HYPER, 0.5, 0.5),
    ];
    this.ballTrails = [
      makeSpriteWithAnchorXY(textures, TEXTURES.BALL_TRAIL, 0.5, 0.5),
      makeSpriteWithAnchorXY(textures, TEXTURES.BALL_TRAIL, 0.5, 0.5),
    ];
    this.punches = [
      makeSpriteWithAnchorXY(textures, TEXTURES.BALL_PUNCH, 0.5, 0.5),
      makeSpriteWithAnchorXY(textures, TEXTURES.BALL_PUNCH, 0.5, 0.5),
    ];
    ...
    this.shadows = {
      ...
      forBalls: [
        makeSpriteWithAnchorXY(textures, TEXTURES.SHADOW, 0.5, 0.5),
        makeSpriteWithAnchorXY(textures, TEXTURES.SHADOW, 0.5, 0.5),
      ],
    };
    ...
    this.container.addChild(this.shadows.forBalls[0]);
    this.container.addChild(this.shadows.forBalls[1]);
    this.container.addChild(this.player1);
    this.container.addChild(this.player2);
    this.container.addChild(this.ballTrails[0]);
    this.container.addChild(this.ballTrails[1]);
    this.container.addChild(this.ballHypers[0]);
    this.container.addChild(this.ballHypers[1]);
    this.container.addChild(this.balls[0]);
    this.container.addChild(this.balls[1]);
    this.container.addChild(this.punches[0]);
    this.container.addChild(this.punches[1]);
    ...
    this.shadows.forBalls[0].y = 273;
    this.shadows.forBalls[1].y = 273;
  ...
  drawPlayersAndBall(physics) {
    ...
    const balls = physics.getActiveBalls
      ? physics.getActiveBalls()
      : physics.balls || [physics.ball];
    ...
    for (let i = 0; i < this.balls.length; i++) {
      ...
      if (ball === undefined) {
        ...
        this.shadows.forBalls[i].visible = false;
        this.ballHypers[i].visible = false;
        this.ballTrails[i].visible = false;
        this.punches[i].visible = false;
        continue;
      }

      this.balls[i].visible = true;
      this.balls[i].x = ball.x;
      this.balls[i].y = ball.y;
      this.shadows.forBalls[i].visible = true;
      this.shadows.forBalls[i].x = ball.x;
      this.balls[i].gotoAndStop(ball.rotation);

      // For punch effect, refer FUN_00402ee0
      if (ball.punchEffectRadius > 0) {
        ball.punchEffectRadius -= 2;
        this.punches[i].width = 2 * ball.punchEffectRadius;
        this.punches[i].height = 2 * ball.punchEffectRadius;
        this.punches[i].x = ball.punchEffectX;
        this.punches[i].y = ball.punchEffectY;
        this.punches[i].visible = true;
      } else {
        this.punches[i].visible = false;
      }

      if (ball.isPowerHit === true) {
        this.ballHypers[i].x = ball.previousX;
        this.ballHypers[i].y = ball.previousY;
        this.ballTrails[i].x = ball.previousPreviousX;
        this.ballTrails[i].y = ball.previousPreviousY;

        this.ballHypers[i].visible = true;
        this.ballTrails[i].visible = true;
      } else {
        this.ballHypers[i].visible = false;
        this.ballTrails[i].visible = false;
      }
