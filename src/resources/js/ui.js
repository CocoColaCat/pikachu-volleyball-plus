...
/** @typedef {{graphic?: string, bgm?: string, sfx?: string, speed?: string, winningScore?: string, ballCount?: string}} Options */
...
export function setUpUI(pikaVolley, ticker) {
  ...
  const applyOptions = (options) => {
    ...
    switch (options.ballCount) {
      case '1':
        pikaVolley.ballCount = 1;
        break;
      case '2':
        pikaVolley.ballCount = 2;
        break;
    }
  };
  ...
  const saveOptions = (options) => {
    ...
    if (options.ballCount) {
      localStorageWrapper.set('pv-offline-ballCount', options.ballCount);
    }
  };
  ...
  const loadOptions = () => ({
    ...
    ballCount: localStorageWrapper.get('pv-offline-ballCount'),
  });
...
function setUpBtns(pikaVolley, applyAndSaveOptions) {
  ...
  const superfastSpeedBtn = document.getElementById('superfast-speed-btn');
  ...
  superfastSpeedBtn.addEventListener('click', () => {
    applyAndSaveOptions({ speed: 'superfast' });
  });
  ...
  const ballCount1Btn = document.getElementById('ball-count-1-btn');
  const ballCount2Btn = document.getElementById('ball-count-2-btn');
  ballCount1Btn.addEventListener('click', () => {
    applyAndSaveOptions({ ballCount: '1' });
  });
  ballCount2Btn.addEventListener('click', () => {
    applyAndSaveOptions({ ballCount: '2' });
  });
...
function setSelectedOptionsBtn(options) {
  ...
  if (options.speed) {
    ...
    switch (options.speed) {
      case 'slow':
        ...
        superfastSpeedBtn.classList.remove('selected');
      ...
      case 'superfast':
        ...
        mediumSpeedBtn.classList.remove('selected');
        fastSpeedBtn.classList.remove('selected');
        superfastSpeedBtn.classList.add('selected');
        break;
    }
  }
  if (options.winningScore) {
  ...
  if (options.ballCount) {
    ...
    switch (options.ballCount) {
      case '1':
        ballCount2Btn.classList.remove('selected');
        ballCount1Btn.classList.add('selected');
        break;
      case '2':
        ballCount1Btn.classList.remove('selected');
        ballCount2Btn.classList.add('selected');
        break;
    }
  }
}
...
function setUpToShowDropdownsAndSubmenus(pikaVolley) {
  ...
  document
    .getElementById('ball-count-submenu-btn')
    .addEventListener('mouseover', () => {
      showSubmenu('ball-count-submenu-btn', 'ball-count-submenu');
    });
  document
